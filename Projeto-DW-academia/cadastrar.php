...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // === 1. Insere o aluno na tabela usuario ===
    $sql = "INSERT INTO usuario (nome, email, senha, idade) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_prepare($conexao, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "sssi", $nome, $email, $senha, $idade);
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            // === 2. Associa o aluno ao treino correspondente ===
            $sql2 = "INSERT INTO usuario_treino (usuario_id_usuario, treino_id_treino) VALUES (?, ?)";
            $stmt2 = mysqli_prepare($conexao, $sql2);

            if ($stmt2) {
                mysqli_stmt_bind_param($stmt2, "ii", $usuario_id, $treino_id);
                mysqli_stmt_execute($stmt2);
                mysqli_stmt_close($stmt2);

                echo "✅ Aluno cadastrado com sucesso como $tipo e associado ao treino $treino_id!";
            } else {
                echo "Erro ao preparar associação de treino: " . mysqli_error($conexao);
            }

        } else {
            echo "Erro ao cadastrar aluno: " . mysqli_stmt_error($stmt);
        }
    } else {
...
<head>
    ...
    <title>Cadastro de Aluno</title>
        ...
        .container {
            ...
            box-shadow: 0 0 25px rgba(57, 255, 20, 0.2); /* Sombra neon sutil */
...
<body>
    ...
    <form method="POST" action="">
        ...
        <input type="date" name="idade" required><br><br>
